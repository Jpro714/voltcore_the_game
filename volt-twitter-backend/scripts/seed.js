...
const avatar = (seed) => `https://api.dicebear.com/7.x/identicon/svg?seed=${seed}`;
const users = [
    {
        id: 'player',
        handle: 'player_one',
...
const followerCount = (userId) => followPairs.filter((pair) => pair.followingId === userId).length;
const followingCount = (userId) => followPairs.filter((pair) => pair.followerId === userId).length;
const basePosts = [
    {
        id: 'tw-1',
        authorId: 'voltcore',
